...
public:
    ...
    explicit RequestHandler(model::Game& game, model::Players& players, Strand api_strand, std::basic_string<char> path_from_arg)
        : game_{game}, players_{players}, api_strand_{api_strand} {
        ...
        if(!decoded_req_target.compare(0, 4, MAP_STORAGE_BASED, 0, 4))
        {
            ...
            auto handle = [self = shared_from_this(), send, req = std::forward<decltype(req)>(req), safe_support](){
                // try {
                        ...
                        send((self->HandleApiRequest(req, self->game_, self->players_, self->static_dir_path, safe_support)));
                    // } catch (...) 
                    // {
            ...
            send(std::move(HandleRequest(std::forward<decltype(req)>(req), game_, players_, static_dir_path, safe_support)));
    ...
    var_t HandleRequest(StringRequest&& req, model::Game& game, model::Players& players, fs::path& stat_folder, std::shared_ptr<struct SupportResponseLogger> supp);
    var_t HandleApiRequest(StringRequest req, model::Game& game, model::Players& players, fs::path& stat_folder, std::shared_ptr<struct SupportResponseLogger> supp);
