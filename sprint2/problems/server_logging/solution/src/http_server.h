...
private:
    ...
    virtual void HandleRequest(HttpRequest&& request, boost::asio::ip::address addr) = 0;
};
...
private:
    ...
    void HandleRequest(HttpRequest&& request, boost::asio::ip::address addr) override {
        // Захватываем умный указатель на текущий объект Session в лямбде,
        // чтобы продлить время жизни сессии до вызова лямбды.
        // Используется generic-лямбда функция, способная принять response произвольного типа
        request_handler_(std::move(request), addr, [self = this->shared_from_this()](auto&& response) {
