                                            ...
                                            application::ApplicationFacade& AppFacade,fs::path& stat_folder, std::shared_ptr<struct SupportResponseLogger> support_log)
    {
            ...
            else
            {
                ...
                // try{
                    ...
                    else // вернуть токен
                    {
                        ...
                        // std::cout << "idx: " << road_idx <<  "roads: " << roads.size() << "del" << del << std::endl;
                        double x_d, y_d;
                        ...
                        auto new_dog = session_ptr->AddDog(name, x_d, y_d);
                        // if(new_dog == nullptr)
                        //     std::cout << "dog null\n";
        ...
        else if(!decoded_req_target.compare(0, PLAYERS_GAME_BASED.size(), PLAYERS_GAME_BASED))
        {
            if(req.method() == http::verb::get || req.method() == http::verb::head)
            {
                ...
                // ret = ExecuteAuthorized([&AppFacade, &req](auto& arg){
                //     return AppFacade.PlayersList(std::forward<decltype(arg)>(arg), req);
                //     // return result;
                // }, req); 
                // try{
