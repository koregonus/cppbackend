...
private:
    ...
    virtual void HandleRequest(HttpRequest&& request, boost::asio::ip::tcp::endpoint endpoint) = 0;
};
...
private:
    ...
    void HandleRequest(HttpRequest&& request, boost::asio::ip::tcp::endpoint endpoint) override {
        // Захватываем умный указатель на текущий объект Session в лямбде,
        // чтобы продлить время жизни сессии до вызова лямбды.
        // Используется generic-лямбда функция, способная принять response произвольного типа
        request_handler_(std::move(request), endpoint, [self = this->shared_from_this()](auto&& response) {
            std::visit([self](auto&& arg){self->Write(std::move(arg));}, response);
