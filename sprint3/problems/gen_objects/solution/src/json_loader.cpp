...
namespace json_loader {
    ...
    static void parse_map(const json::value& map, model::Game& game, double defaultDogSpeed, app_support::FrontendExtraDataMap& extra_data)
    {
        ...
        {
            local_extra_map->count = map.as_object().at("lootTypes").as_array().capacity();
            local_extra_map->lootTypes = std::make_shared<json::array>(map.as_object().at("lootTypes").as_array());
        }
    ...
    model::Game LoadGame(const std::filesystem::path& json_path, app_support::FrontendExtraDataMap& extra_data) {
        ...
        for(int i = 0; i < mp.as_array().capacity(); i++)
        {
            ...
            parse_map(ref_map, ref_game, defaultDogSpeed, extra_data);
        }
        return game;
    }
