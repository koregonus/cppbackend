...
SCENARIO("Controller", "[Controller]") {
    ...
    GIVEN("Controller and TV") {
        ...
        WHEN("The TV is turned off") {
            ...
            AND_WHEN("SelectChannel entered when TV is off") {
                run_menu_command("SelectChannel 3");

                THEN("the tv is off message printed") {
                    expect_output("TV is turned off\n"s);
                }
            }

            AND_WHEN("SelectPrevChannel entered when TV is off") {
...
// #if 0
            ...
            AND_WHEN("Entered wrong channel num") {
                run_menu_command("SelectChannel 3.0"s);
                // run_menu_command("Info"s);

                THEN("invalid argument is printed") {
                    expect_output("Invalid channel\n"s);
                }
            }

            AND_WHEN("Info entered after SelectPreviousChannel after a SelectChannel") {
            ...
            AND_WHEN("Info entered after SelectPreviousChannel after a 2 x SelectChannel") {
                run_menu_command("SelectChannel 3"s);
                run_menu_command("SelectChannel 4"s);
                run_menu_command("SelectPreviousChannel"s);
                run_menu_command("Info"s);
            ...
            AND_WHEN("Info entered after 2 x SelectPreviousChannel after a 2 x SelectChannel") {
                run_menu_command("SelectChannel 3"s);
                run_menu_command("SelectChannel 4"s);
                run_menu_command("SelectPreviousChannel"s);
                run_menu_command("SelectPreviousChannel"s);
