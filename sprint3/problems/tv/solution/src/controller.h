...
private:
    ...
    [[nodiscard]] bool ShowInfo(std::istream& input, std::ostream& output) const {
        ...
        if (EnsureNoArgsInInput(INFO_COMMAND, input, output)) {
            if (tv_.IsTurnedOn()) {
                ...
                Channel number is <номер канала>
                */
                std::optional<int> ch = tv_.GetChannel();
                output << "TV is turned on" << std::endl;
                output << "Channel number is " << *ch << std::endl;
    ...
    [[nodiscard]] bool SelectChannel(std::istream& input, std::ostream& output) const {
        ...
        try {
            ...
            channel = std::stoi(input_buf, &index);
            if(index > 0)
            {
                if(std::isdigit(std::atoi(&input_buf[0])))
                {
                    output << std::atoi(&input_buf[0]) << std::endl;
                    output << std::isdigit(std::atoi(&input_buf[0])) << std::endl;
