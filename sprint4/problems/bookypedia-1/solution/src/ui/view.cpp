...
bool View::AddBook(std::istream& cmd_input) const {
    try {
        if (auto params = GetBookParams(cmd_input)) {
            // std::cout << "bp2\n";
            // std::cout << (*params).author_id << std::endl;
            // std::cout << (*params).title << std::endl;
            // std::cout << (*params).publication_year << std::endl;
...
std::vector<detail::BookInfo> View::GetBooks() const {
    ...
    {
        books.push_back({all_books[i].first, all_books[i].second});
    }
    return books;
}
