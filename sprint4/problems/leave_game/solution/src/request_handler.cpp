                                            ...
                                            application::ApplicationFacade& AppFacade,fs::path& stat_folder, std::shared_ptr<struct SupportResponseLogger> support_log)
    {
        ...
        else if(!decoded_req_target.compare(0, RECORDS_BASED.size(), RECORDS_BASED))
        {
            if(decoded_req_target.size() > RECORDS_BASED.size())
            {
                ...
                start = str_params.substr(start_pos,start_width);
                delim_pos++;
                while(str_params[delim_pos] != '=')
                {
                    delim_pos++;
                ...
                delim_pos++;
                maxItem = str_params.substr(delim_pos);
                int start_digit = std::stoi(start);
                int maxItem_digit = std::stoi(maxItem);
                if(maxItem_digit > 100)
                {
                    ret = json_response(http::status::bad_request, R"({"code": "badRequest", "message": "Bad Request"})"sv, true, AllowedMethods::ALLOW_GET_HEAD); 
                }
                else
                {
                    ret = AppFacade.GetRecordsFromDB(req, start_digit, maxItem_digit);
                }
