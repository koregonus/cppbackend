...
StringResponse ApplicationFacade::GetRecordsFromDB(const StringRequest& req, int start, int max_items)
{
	StringResponse ret;
	std::vector<model::DogLeftDump> check_dogs;
	auto connect_db = conn_pool_.GetConnection();
	postgres::ReadScoresFromDatabase(*connect_db, start, max_items, check_dogs); 
	if(check_dogs.size() > 0)
	{
		...
		{
			json::object local_obj;
			local_obj["name"] = iter->name;
    		local_obj["score"] = iter->score;
    		local_obj["playTime"] = iter->play_time_ms;
    		records.push_back(local_obj);
		}
		std::string str;
		str = json::serialize(records);
    	std::string_view sv_str(str);
    	ret = MakeStringResponse(http::status::ok, std::move(sv_str), req.version(), req.keep_alive(),
    								 req.method(), true, ContentType::APP_JSON, AllowedMethods::ALLOW_GET_HEAD);
	}
