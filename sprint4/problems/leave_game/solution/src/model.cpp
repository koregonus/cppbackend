...
void GameSession::EraseDogById(int id)
{
    for(auto it = dogs_.begin(); it != dogs_.end();)
    {
        auto dog_id = (*it)->GetId();
        if(*dog_id == id)
        {
            dogs_.erase(it);
            return;
...
std::vector<model::DogLeftDump> Players::CheckPlayers(double limit)
{
    std::vector<model::DogLeftDump> ret;
