...
namespace json_loader {
    ...
    static void parse_map(const json::value& map, model::Game& game, double defaultDogSpeed, 
                          int64_t defaultBagCapacity, double defaultDogRetirementTime, app_support::FrontendExtraDataMap& extra_data)
    {
    ...
    model::Game LoadGame(const std::filesystem::path& json_path, app_support::FrontendExtraDataMap& extra_data) {
        ...
        double defaultDogRetirementTime = 60.0;
        ...
        if(value.as_object().if_contains("dogRetirementTime"))
        {
            defaultDogRetirementTime = value.at("dogRetirementTime").as_double();
        }
        ...
        for(int i = 0; i < mp.as_array().capacity(); i++)
        {
            ...
            parse_map(ref_map, ref_game, defaultDogSpeed, defaultBagCapacity, defaultDogRetirementTime, extra_data);
        }
        return game;
    }
